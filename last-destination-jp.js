...
document.getElementById("tool-bar-phone").style.top = "-275px";
}
if (prevScrollpos > currentScrollPos) {
...
document.querySelectorAll(".fade-carousel").forEach(carousel => {
  const slides = carousel.querySelectorAll(".fade-slide");
  ...
  carousel.addEventListener("touchend", e => {
    ...
    if (delta < -40) index = (index + 1) % slides.length;
    if (delta > 40) index = (index - 1 + slides.length) % slides.length;
